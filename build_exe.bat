...
echo.
echo === Icon Copy Table : build EXE ===
echo.
...
taskkill /F /IM IconCopyTable.exe >nul 2>&1
timeout /t 2 /nobreak >nul

if exist "dist\IconCopyTable.exe" (
  del /f /q "dist\IconCopyTable.exe" >nul 2>&1
)
if exist "dist\IconCopyTable.exe" (
  echo [ERROR] Cannot replace dist\IconCopyTable.exe
...
if errorlevel 1 (
  echo [WARN] icon.ico was not written; continuing without a custom icon.
)
...
echo.
echo OK
echo EXE: "%cd%\dist\IconCopyTable.exe"
echo.
if exist "dist\IconCopyTable.exe" explorer dist
pause
